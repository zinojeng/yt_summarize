...
class YouTubeSummarizer:
    ...
    def prepare_summary_prompt(self, transcript: str, video_title: str = "") -> str:
        ...
        # Updated prompt with corrected bold formatting instruction
        prompt_template = f"""
# 指令：優化與摘要記錄 (超緊湊版)
...
2.  **格式與排版要求** (請嚴格遵守，目標是【極度緊湊】)
    ...
    *   **分隔線**: *僅在* 主要區塊之間 (例如摘要和洞察之間) 使用 `---` 分隔線。
    *   **粗體**: 
        *   僅使用 `**粗體**` 標示 **區塊標題本身** (例如：`## **重點摘要**`)。
        *   **不要** 將緊跟在「重點摘要」、「關鍵洞察」、「主題關鍵字」標題後面的文字段落或列表項設為粗體。
        *   文本中的其他**關鍵詞**可以設為粗體。
    *   **列表**: 使用 `-` 或 `*` 製作項目清單。
    *   **引用**: 適當使用 `>` 引用*重要語句*。
    *   **斜體**: 適當使用 `*斜體*` 強調*次要重點*。
    *   **【嚴格控制空行】**: 
        *   **標題後禁止空行**: 標題 (如 `## **重點摘要**`) 下方 **不應有** 任何空白行。
...
請嚴格按照以下結構和 Markdown 格式生成內容，所有內容均為**繁體中文**，並確保**極度緊湊**，且**僅標題加粗**：

## **重點摘要**
(此處直接填寫 300 字以內的摘要內容，非粗體)
---
## **關鍵洞察**
-(洞察點 1，非粗體)
-(洞察點 2，非粗體)
